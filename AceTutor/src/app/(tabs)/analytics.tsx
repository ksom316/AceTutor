...
export default function AnalyticsScreen() {
  ...
  const scoreTrend = useMemo(
    () => attempts.map((a) => (a.total ? Math.round((a.score / a.total) * 100) : 0)),
    [attempts],
  );
    ...
    <Screen>
      ...
      {!hasData ? (
        <EmptyState
          ...
          body={loading ? undefined : "Start a lesson or take a quiz and your insights will appear here."}
        />
      ) : (
        <>
          {/* KPIs */}
            ...
            <Card style={{ gap: 14 }}>
              ...
              {perCourse.map((p, i) => (
                <View key={p.name} style={{ gap: 6 }}>
                  ...
                  <View style={{ height: 8, borderRadius: 999, backgroundColor: c.surface, overflow: "hidden" }}>
                    <View
                      style={{
              ...
              {perCourse.map((p, i) => {
                ...
                return (
                  <View key={p.name} style={{ flexDirection: "row", alignItems: "center", gap: 12 }}>
                    <View style={{ width: 6, height: 36, borderRadius: 3, backgroundColor: PALETTE[i % PALETTE.length] }} />
