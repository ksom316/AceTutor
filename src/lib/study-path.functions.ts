...
export const STUDY_PATH_ERRORS: Record<string, string> = {
  ...
  NOT_A_MODULE_QUIZ:
    "A study path can only be built from a module quiz, not a General Course Quiz.",
  ...
  INSUFFICIENT_CONTENT:
    "This module doesn't have enough written material yet to build a study path.",
...
function parseStudyPath(raw: string): StudyPathContent | null {
...
async function runGeneration(payload: AiPayload): Promise<StudyPathContent> {
  const system =
    "You build a SHORT, targeted remedial study path for ONE university module. You are given the " +
    "EXACT questions a student answered incorrectly, plus the module's own material. Group the " +
    ...
    "shown by those questions. Do not write a full replacement course. Prefer the supplied module " +
  ...
  const user = [
    ...
    "",
    "MODULE MATERIAL (prefer this; may be empty):",
    "<<<",
    payload.material ||
    ...
    '{ "title": "Personalized Study Path", "weakAreas": [ { "title": string, "explanation": string, "example": string, "practice": [ { "question": string, "answer": string } ] } ] }',
...
export const generateStudyPath = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<GenerateStudyPathResult> => {
    ...
    // caller, so another student's attempt id simply returns nothing.
    ...
    if (!attempt.topic_id) throw new Error("NOT_A_MODULE_QUIZ"); // General Course Quiz → rejected
    ...
    const { data: existing } = await supabase
      .from("study_paths")
      .select(
        "id, user_id, topic_id, attempt_id, weak_question_ids, content, created_at, completed_at",
      )
    ...
    // 9. Module info + analysable lesson material.
    ...
    const material = buildModuleMaterial((lessons ?? []) as LessonForCapability[]);
    const hasExplanations = incorrect.some(
      (r) => (r.questions?.explanation ?? "").trim().length > 0,
    );

    // A genuinely empty module (no analysable material, no summary, and no
    // question explanations) has nothing to teach from beyond raw prompts.
    if (
      material.length < MIN_ANALYSABLE_CHARS &&
      !(topic?.summary ?? "").trim() &&
      !hasExplanations
    ) {
    ...
    const payload: AiPayload = {
      moduleTitle: topic?.title ?? "This module",
      moduleSummary: (topic?.summary ?? "").trim() || null,
      material,
