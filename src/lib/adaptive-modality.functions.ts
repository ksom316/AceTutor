...
/**
 * The A7 recommendation compute, extracted so other server code (R1 remedial
 * content) can reuse the EXACT same logic without a second implementation. The
 * algorithm, reads, evidence windows and thresholds are unchanged — this is a
 * pure refactor of the server-fn handler below.
 */
export async function resolveAdaptiveModalityRecommendation(
  supabase: SupabaseClient<Database>,
  userId: string,
  topicId: string,
): Promise<AdaptiveModalityRecommendation> {
  const [{ data: lessons }, { data: profile }, { data: outcomeRows }, { data: interactionRows }] =
    await Promise.all([
      supabase.from("lessons").select("modality").eq("topic_id", topicId),
      supabase
        .from("vark_profiles")
...
export const getAdaptiveModalityRecommendation = createServerFn({ method: "POST" })
  ...
  .handler(
    ({ data, context }): Promise<AdaptiveModalityRecommendation> =>
